...
- `justfile_directory()` - Retrieves the parent directory of the justfile being executed.

For example, to call `rustfmt` on files under the same directory of the justfile, use the following rule:

```
rustfmt:
    find {{justfile_directory()}} -name \*.rs -exec rustfmt {} \;
```

Alternatively, if your command needs to be run at the same location as the justfile, you could use (e.g.):

```
build:
    cd {{justfile_directory()}}; ./some_script_that_needs_to_be_run_from_here
```
