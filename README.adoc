...
As of `just` 0.11.0, the `positional-arguments` setting defaults to `true`. Positional arguments can be turned off with `set positional-arguments := false`.
...
Recipe arguments maybe accessed using `{{…}}` syntax and `just` expressions, or as positional arguments:
...
Expressions inside `{{…}}` are checked at compile time. However, they are subject to evaluation and splitting by the shell. This can make passing arguments containing that are treated specially by the shell tricky, for example `"'* !`. To avoid this, use quoted positional arguments, for example `"$1"` and `"$@"`.
