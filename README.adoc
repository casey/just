...
Settings can be used to control various aspects of interpetation and execution. (This is aspirational. There is only one setting at the moment.)
...
foo:
  # this line will be executed as `zsh -cu 'ls **/*.txt'`
  ls **/*.txt
```

Each setting can be given at most once, anywhere in a justfile. Setting the same setting multiple times is an error.
...
The `shell` setting controls the command used to interpret recipe lines and backticks. Shebang recipes are unaffected.
