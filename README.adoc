...
- `clean(path)` - Simplify `path` by removing extra path separators, intermediate `.` components, and removing `..` where possible. `clean("foo//bar")` is `foo/bar`, `clean("foo/..")` is `.`, `clean("foo/./bar")` is `foo/bar`.
