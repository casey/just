...
a:"#,
    ...
    .stderr(
      r#"Error: `\q` is not a valid escape sequence
   ╭─[ justfile:1:6 ]
   │
 1 │ x := "\q"
───╯
...
fn unicode_escape_no_braces() {
  Test::new()
    ...
    .args(["--evaluate", "x"])
    .stderr(
      r#"Error: expected unicode escape sequence delimiter `{` but found `1`
   ╭─[ justfile:1:6 ]
   │
 1 │ x := "\u1234"
───╯
...
fn unicode_escape_empty() {
  Test::new()
    ...
    .args(["--evaluate", "x"])
    .stderr(
      r#"Error: unicode escape sequences must not be empty
   ╭─[ justfile:1:6 ]
   │
 1 │ x := "\u{}"
───╯
...
fn unicode_escape_requires_immediate_opening_brace() {
  Test::new()
    ...
    .args(["--evaluate", "x"])
    .stderr(
      r#"Error: expected unicode escape sequence delimiter `{` but found ` `
   ╭─[ justfile:1:6 ]
   │
 1 │ x := "\u {1f916}"
───╯
...
fn unicode_escape_unterminated() {
  Test::new()
    ...
    .args(["--evaluate", "x"])
    .stderr(
      r#"Error: unterminated unicode escape sequence
   ╭─[ justfile:1:6 ]
   │
 1 │ x := "\u{1f917"
───╯
