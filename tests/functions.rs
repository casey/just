...
fn join_argument_count_error() {
  Test::new()
    ...
    .args(["--evaluate"])
    .stderr(
      r"Error: Function `join` called with 1 argument but takes 2 or more
   ╭─[ justfile:1:6 ]
   │
 1 │ x := join('a')
───╯
...
fn shell_no_argument() {
  Test::new()
    ...
    .args(["--evaluate"])
    .stderr(
      r"Error: Function `shell` called with 0 arguments but takes 1 or more
   ╭─[ justfile:1:8 ]
   │
 1 │ var := shell()
───╯
...
fn unary_argument_count_mismamatch_error_message() {
  Test::new()
    ...
    .args(["--evaluate"])
    .stderr(
      r"Error: Function `datetime` called with 0 arguments but takes 1
   ╭─[ justfile:1:6 ]
   │
 1 │ x := datetime()
───╯
