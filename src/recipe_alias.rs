...
pub(crate) struct RecipeAlias<'src, T = Arc<Recipe<'src>>> {
...
impl<'src> RecipeAlias<'src, Namepath<'src>> {
...
impl<'src, T> Keyed<'src> for RecipeAlias<'src, T> {
  fn key(&self) -> &'src str {
    self.name.lexeme()
  }
}

impl<'src> Display for RecipeAlias<'src, Namepath<'src>> {
