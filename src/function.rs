...
pub(crate) enum Function {
  ...
  List(fn(Context, &Value) -> ValueResult),
  UnaryMap(fn(Context, &str) -> StringResult),
  ListListOpt(fn(Context, &Value, Option<&Value>) -> ValueResult),
  UnaryPlus(fn(Context, &str, &[String]) -> StringResult),
  UnaryValue(fn(Context, &str) -> ValueResult),
  Binary(fn(Context, &str, &str) -> StringResult),
  StrList(fn(Context, &str, &Value) -> ValueResult),
  ListList(fn(Context, &Value, &Value) -> ValueResult),
...
impl Function {
  pub(crate) fn expected_arguments(&self) -> RangeInclusive<usize> {
    match *self {
      ...
      Unary(_) | List(_) | UnaryMap(_) | UnaryValue(_) => 1..=1,
      ListListOpt(_) => 1..=2,
      UnaryPlus(_) => 1..=usize::MAX,
      Binary(_) | StrList(_) | ListList(_) | BinaryValue(_) => 2..=2,
...
pub(crate) fn get(name: &str) -> Option<Function> {
  ...
  let function = match name.as_str() {
    ...
    "env" => ListListOpt(env),
    "env_var" => List(env_var),
    "env_var_or_default" => ListList(env_var_or_default),
    "error" => Unary(error),
